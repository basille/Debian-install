#+TITLE: Installation de Debian 13
#+AUTHOR: Mathieu Basille


Ceci est une mise-à-jour du tutoriel complet pour installer Debian en version 13
stable (« Trixie ») sur un portable Dell Precision 3581. La plupart des
...
- [[#opérations-systèmes][Opérations systèmes]]
  ...
  - [[#mettre-à-jour-debian-vers-la-version-13-trixie][Mettre à jour Debian vers la version 13 (Trixie)]]
...
### Dépôts officiels Debian pour Trixie
...
echo "deb [signed-by=/etc/apt/trusted.gpg.d/cran_debian_key.asc] https://cloud.r-project.org/bin/linux/debian trixie-cran40/" | sudo tee -a /etc/apt/sources.list.d/r.list
# QGIS
...
sudo apt install \
    ...
    `# Utilitaires` cmake git gparted htop mlocate net-tools python3-pip rename sqlite3 fwupd tlp tlp-rdw localepurge flatpak gnome-software-plugin-flatpak ripgrep \
    ...
    `# Libre Office` libreoffice-style-sifr/trixie-backports \
...
sudo apt install \
    `# Libre Office` -t trixie-backports libreoffice-style-sifr
#+end_src
...
=chrome/firefox-gnome-theme=, puis on met à jour le dépôt et on redémarre
Firefox :
...
On suit les [[https://qgis.org/resources/installation-guide/#debian--ubuntu][instructions officielles pour Debian/Ubuntu]], puis on lance QGIS et
...
** Mettre à jour Debian vers la version 13 (Trixie)

On suit les instructions détaillées indiquées [[https://lecrabeinfo.net/tutoriels/mettre-a-jour-debian-12-bookworm-vers-debian-13-trixie/][ici]]. Les étapes sont les
suivantes :

1) Sauvegarder ses données
2) Mettre à jour son système Debian 12
3) Vérifier les paquets non officiels et bloqués
4) Modifier le fichier sources.list
5) Mettre à niveau Debian 12 vers Debian 13
6) Redémarrer le système
7) Nettoyer le système (facultatif)
